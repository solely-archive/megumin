...
Megumin.on('message', (msg) => {
  ...
  const command =
    Megumin.commands.get(commandName) ||
  ...
  if (command.disabled) {
    let embed = new MessageEmbed()
      ...
      .setColor(0xff0000)
      .setDescription(
        `Command \`${command.name}\` has been disabled. Please wait some time, until the bot owner(s) decide to enable it.`
      );
  ...
  try {
    ...
    let embed = new MessageEmbed()
      ...
      .setColor(0xff0000)
      .setDescription(
        `An error occured for command ${command.name}. Please contact <@${config.owner}> with this error: \`${err.message}\`.`
      );
    return msg.channel.send(embed);
  }
});
